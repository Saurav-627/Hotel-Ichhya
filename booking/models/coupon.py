...
class Coupon(models.Model):
    ...
    APPLICABLE_TYPES = (
        ('all', 'All Products (Rooms & Services)'),
    ...
    min_spend = models.DecimalField(max_digits=10, decimal_places=2, default=0.00, help_text="Default minimum spend in base currency")
    # Redemption limits
    ...
    def is_valid(self, order_amount=0, product_type=None, active_currency_code=None):
        ...
        if product_type and self.applicable_to != 'all' and self.applicable_to != product_type:
            target = "Room Stays" if self.applicable_to == 'room' else "Services"
        ...
        elif self.min_spend and Decimal(str(order_amount)) < self.min_spend:
            return False, f"Minimum spend of {self.min_spend} required to apply this coupon."
