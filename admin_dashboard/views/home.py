...
class DashboardHomeView(StaffRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # 1. Today's Check-ins / Check-outs (Confirmed/Checked-in only, excluding draft/pending/cancelled)
        ...
        # 2. Room Occupancy
        total_rooms_capacity = Room.objects.filter(is_published=True).aggregate(total=Sum('total_rooms'))['total'] or 0
        ...
        # 3. Booking Stats (Confirmed & Checked In)
        # pyrefly: ignore [missing-attribute]
        confirmed_bookings_count = Booking.objects.filter(status='confirmed').count()
        # pyrefly: ignore [missing-attribute]
        checked_in_bookings_count = Booking.objects.filter(status='checked_in').count()
        # pyrefly: ignore [missing-attribute]
        total_valid_bookings_count = Booking.objects.filter(status__in=['confirmed', 'checked_in', 'checked_out']).count()
        ...
        # 11. Recent Activities List (Excludes draft/pending bookings)
        # pyrefly: ignore [missing-attribute]
        ...
        context.update({
            ...
            'checked_in_bookings_count': checked_in_bookings_count,
            'total_valid_bookings_count': total_valid_bookings_count,
